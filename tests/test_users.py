...
class TestUserPreference:
    ...
    def test_custom_preferences(self):
        ...
        assert prefs.watched_stocks == ["sh600519", "HK00700"]
    ...
    def test_preferences_model_dump(self):
        prefs = UserPreference(
            ...
            watched_stocks=["sh600519"]
        )
        dump = prefs.model_dump()
        assert dump["watched_industries"] == ["科技"]
        assert dump["watched_stocks"] == ["sh600519"]
...
class TestUserCreate:
    ...
    def test_create_user_with_preferences(self):
        prefs = UserPreference(watched_stocks=["sh600519"])
        request = UserCreate(
            email="test@example.com",
            preferences=prefs
        )
        assert request.preferences.watched_stocks == ["sh600519"]
...
class TestUserUpdate:
    ...
    def test_update_preferences_only(self):
        prefs = UserPreference(watched_stocks=["HK00700"])
        update = UserUpdate(preferences=prefs)
        assert update.preferences.watched_stocks == ["HK00700"]
...
class TestUserService:
    ...
    def test_create_user(self, mock_get_redis):
        ...
        request = UserCreate(
            ...
            preferences=UserPreference(
                watched_stocks=["sh600519", "HK09988"],
                watched_industries=["科技"]
            )
        )
        ...
        assert user.email == "test@example.com"
        assert user.name == "Test User"
        assert user.preferences.watched_stocks == ["sh600519", "HK09988"]
        assert user.is_active is True
    ...
    def test_update_preferences(self, mock_get_redis):
        ...
        assert updated.preferences.watched_stocks == ["sh600519", "HK00700"]
    ...
    def test_get_users_by_stock(self, mock_get_redis):
        ...
        user2 = UserProfile(
            user_id="u2",
            email="user2@example.com",
            preferences=UserPreference(watched_stocks=["HK09988"])
        )
        ...
        service = UserService(redis_client=mock_redis)
        users = service.get_users_by_stock("sh600519")
...
class TestEmailService:
    ...
    def test_render_html_report_morning(self):
        ...
        report = DailyReport(
            user=test_user,
            stock_summaries=[
                {"symbol": "sh600519", "name": "贵州茅台", "price": 250.0, "change": 2.5, "news": [{"title": "Test News"}]}
            ],
            industry_summaries=[],
