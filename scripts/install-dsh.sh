...
echo ""
echo "建议 Settings.dsh_npm_bin 值（可直接 exec 的 JSON-RPC 运行时目标）:"
echo "  DSH_NPM_BIN=\"$NODE_BIN $JSONRPC_BIN\""
echo "启动示例（argv[2] 或 DSH_CORDIS_CONFIG 指向组合配置）:"
echo "  $NODE_BIN $JSONRPC_BIN <仓库根>/cordis.finana.yml"
echo "配套环境变量:"
echo "  DSH_SYSTEM_PROMPT=<loader 注入的系统提示词>"
